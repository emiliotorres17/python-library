...
"""========================================================================
Purpose:
    The purpose of this script is to perform the back tracking using the
    RK4 time integration method and the second order linear interpolation.
...
def ghost_cells(
    ...
    for k in range(0, N+2):
        for j in range(0, N+2):
            field[0,j,k]    = field[N,j,k]
            field[N+1,j,k]   = field[1,j,k]
    #---------------------------------------------------------------------#
    # x2 direction points                                                 #
    #---------------------------------------------------------------------#
    for k in range(0, N+2):
        for i in range(0, N+2):
            field[i,0,k]    = field[i,N,k]
            field[i,N+1,k]   = field[i,1,k]
    #---------------------------------------------------------------------#
    # x3 direction points                                                 #
    #---------------------------------------------------------------------#
    for j in range(0, N+2):
        for i in range(0, N+2):
            field[i,j,0]    = field[i,j,N]
            field[i,j,N+1]   = field[i,j,1]
...
def rk4_back_tracking(
    ...
    for i in range(tf, tn, -1):
        ...
        #print('k31 --> %7.4e'           %(k31))
        #print('\n')
        #print('x1 --> %7.4e'            %(x1_0 - 0.5*dt*k11))
        #print('x2 --> %7.4e'            %(x2_0 - 0.5*dt*k21))
        #print('x3 --> %7.4e'            %(x3_0 - 0.5*dt*k31))
        #print('t --> %7.4e'             %(t - 0.5*dt))
...
if __name__ == '__main__':
    ...
    u3      = ghost_cells(u3)
    print('\tvelocity-3')
    time    = load(data_path + 'time.npy')[:701]
    print('\ttime')
    #---------------------------------------------------------------------#
    # Domain variables                                                    #
    #---------------------------------------------------------------------#
    dx  = 2.0*pi/float(128)
    x   = linspace(-0.5*dx, 2.0*pi+0.5*dx, 130)
    y   = linspace(-0.5*dx, 2.0*pi+0.5*dx, 130)
    z   = linspace(-0.5*dx, 2.0*pi+0.5*dx, 130)
    #---------------------------------------------------------------------#
    # Initial start point                                                 #
    #---------------------------------------------------------------------#
    t0  = 4.64610535852565575965
    x0  = 5.88948518647765428824
    y0  = 5.34159535296485277200
    z0  = 5.44176721906742155710
    #---------------------------------------------------------------------#
    # Back tracking                                                       #
    #---------------------------------------------------------------------#
